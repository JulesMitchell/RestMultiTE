# Usage: Rscript oktos_task_asymmetry_modelfit.R <task> <model_index> <working_dir> e.g.
# Usage: Call oktos_task_asymmetry_modelcomp.pbs
...
if (!grepl("^[0-4]$", args[2])) {
  stop("Model index must be a number between 0 and 4")
}
...
formula1 <- bf(
  asymmetry_transformed ~ 0 + Intercept + group * timepoint + (1 || subject:regions), # returns coefficients for the beta distribution’s mean parameter (mu)
  phi ~ group * timepoint + (1 | subject:regions), 
...
formula2 <- bf(
  asymmetry_transformed ~ 0 + Intercept + group * timepoint + (1 + timepoint || subject:regions), 
  phi ~ group * timepoint + (1 | subject:regions), 
  zoi ~ group * timepoint + (1 | subject:regions), 
  coi ~ group * timepoint + (1 | subject:regions),
  family = zero_one_inflated_beta()
...
formula3 <- bf(
  asymmetry_transformed ~ 0 + Intercept + group * timepoint + (1 + group || subject:regions),
  phi ~ group * timepoint + (1 | subject:regions), 
  zoi ~ group * timepoint + (1 | subject:regions), 
  coi ~ group * timepoint + (1 | subject:regions),
  family = zero_one_inflated_beta()
...
if (model_index == 0) {
  model <- brm(
    ...
    seed = 22,
    file = paste0(base_path, trial_type_, "_asymmetry_model1"),
    control = control
  )
  ...
  model <- brm(
    ...
    save_pars = save_pars(all = TRUE),
    seed = 22,
    file = paste0(base_path, trial_type_, "_asymmetry_model2"),
    control = control
  )
  ...
  model <- brm(
    ...
    save_pars = save_pars(all = TRUE),
    seed = 22,
    file = paste0(base_path, trial_type_, "_asymmetry_model3"),
    control = control
  )
  ...
  model <- brm(
    ...
    save_pars = save_pars(all = TRUE),
    seed = 22,
    file = paste0(base_path, trial_type_, "_asymmetry_model4"),
    control = control
  )
} else {
