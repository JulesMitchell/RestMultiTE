...
model_summary_output <- "c:/Users/j_m289/Pictures/phd/3. Data Analysis/studies/OKTOS/resting_te/analysis/brms/output/asymmetry_model_summary.xlsx"
model_marginal_effects_output <- "c:/Users/j_m289/Pictures/phd/3. Data Analysis/studies/OKTOS/resting_te/analysis/brms/output/asymmetry_marginaleffects.xlsx"
response_marginal_diff_output <- "c:/Users/j_m289/Pictures/phd/3. Data Analysis/studies/OKTOS/resting_te/analysis/brms/output/asymmetry_resp_marginaldiff.csv"
timepoint_marginal_diff_output <- "c:/Users/j_m289/Pictures/phd/3. Data Analysis/studies/OKTOS/resting_te/analysis/brms/output/asymmetry_timepoint_marginaldiff.csv"
...
marginal_means <- fitted_draws %>%
  group_by(group, timepoint, condition, regions) %>%
  summarise(
    mean = mean(.epred),
...
# Plot Marginal Means and Highest Density Intervals
pd <- position_dodge(0.5)  # Set the dodge width to 0.1 to move points and error bars horizontally

fig1 <- marginal_means %>%
  ggplot(aes(x = regions, y = mean, color = group)) +
  ...
  # Labels and title
  labs(x = "Region Pairs", y = "Asymmetry", color = "Group") +  # Change color legend title to 'Group'
  # Use a light theme with gridlines and minimal background
  theme_light() +
  ...
  # Facet by both 'timepoint' and 'condition' using a grid layout
  facet_grid(timepoint ~ condition) +
  # Change facet text color to black
  theme(
    # remove the vertical grid lines
    panel.grid.major.x = element_blank(),
    strip.text = element_text(color = "black", size = 12),  # Customize facet label text color to black
...
fig2 <- marginal_means %>%
  ggplot(aes(x = regions, y = mean, color = timepoint)) +
  # Points for the estimated values
  geom_point(size = 3, position = pd) +
  # Error bars for the confidence intervals
  geom_errorbar(aes(ymin = HDI_low_0.95, ymax = HDI_high_0.95), width = 0.2, position = pd) +
...
timepoint_RESP_EC_comp <- fitted_draws %>%
	filter(group != "NR") %>%
	filter(condition == "EC") %>%
    group_by(timepoint, regions, .draw) %>%
    summarise(
        .epred = mean(.epred),
        .groups = "drop"
    ) %>%
...
## ----- Data Export --------------
# Export tables
write.xlsx(effects, model_summary_output, sheetName="asymmetry_EffectEstimates", append=TRUE)
write.xlsx(pvalues, model_summary_output, sheetName="asymmetryPvalues",  append=TRUE)
write.xlsx(marginal_means, model_marginal_effects_output, sheetName="asymmetryMarginalmeans",  append=TRUE)
...
## ---- Save figures -----------
ggsave("c:/Users/j_m289/Pictures/phd/3. Data Analysis/studies/OKTOS/resting_te/analysis/brms/figures/asymmetry/asymmetry_interaction_1.jpeg", plot = fig1, width = 10, height = 10)
ggsave("c:/Users/j_m289/Pictures/phd/3. Data Analysis/studies/OKTOS/resting_te/analysis/brms/figures/asymmetry/asymmetry_interaction_2.jpeg", plot = fig2, width = 10, height = 10)