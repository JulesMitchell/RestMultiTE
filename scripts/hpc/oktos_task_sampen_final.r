...
# PBS: Run oktos_task_sampen_modelcomp.pbs
...
if (model_index == 0) {
  model <- brm(
    ...
    seed = 22,
    file = paste0(base_path, task_, "_finalmodel_prior"),
    control = control
  )
  ...
  model <- brm(
    ...
    save_pars = save_pars(all = TRUE),
    seed = 22,
    file = paste0(base_path, task_,  "_finalmodel"),
    control = control
  )
} else {
...
cat("Final sample entropy model fitting completed for model index:", model_index, "\n")